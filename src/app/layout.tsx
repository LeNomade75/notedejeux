...
export const metadata = { title: "Critiques du serveur", description: "Les avis jeux, films, séries et animes de la communauté" };
    ...
    <html lang="fr" className={`dark ${display.variable} ${body.variable}`}>
      <body>
            ...
            <div className="flex items-center gap-4 text-sm">
              {session ? (
                <>
                  ...
                  <Link href="/profile" className="text-muted hover:text-ink">Mon profil</Link>
                  <SignOutButton />
                </>
              ) : (
                <SignInButton />
        ...
        <footer className="mx-auto max-w-6xl px-4 pb-8 text-xs text-muted">
          Données : IGDB (jeux), TMDB (films et séries), AniList (animes). Ce site utilise l'API TMDB mais n'est ni soutenu ni certifié par TMDB.
        </footer>
      </body>
    </html>
  );
